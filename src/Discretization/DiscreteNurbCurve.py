...
import sys
import itertools
import Discretization.Utilities as utils
    ...
    **Examples**
    ...
    def ctrlptsw(self):
        ...
        for c, w in itertools.product(self._mCtrlPts, self._mWeights):
            temp_list = (float(c[0] * w), float(c[1] * w), float(c[2] * w), float(w))
        ...
        try:
            ...
            with open(filename, 'r') as fp:
                for line in fp:
                    ...
                    coords = line.split(',')
                    # Remove extra whitespace and convert to float
                    pt = [float(coords[0].strip()) / float(coords[3].strip()),
                          float(coords[1].strip()) / float(coords[3].strip()),
                    ...
                    self._mWeights.append(float(coords[3].strip()))
        except IOError:
            print('Cannot open file ' + filename)
            sys.exit(1)
    ...
    def evaluate(self):
        ...
        for u in utils.frange(0, 1, self._mDelta):
            ...
            for i in range(0, self._mDegree + 1):
                ...
                curvept[2] += (basis[i] * self._mCtrlPts[span - self._mDegree + i][2])
    ...
    def evaluate_rational(self):
        ...
        for u in utils.frange(0, 1, self._mDelta):
            ...
            for i in range(0, self._mDegree + 1):
                ...
                curveptw[2] += (basis[i] * self._mCtrlPts[span - self._mDegree + i][2] * self._mWeights[span - self._mDegree + i])
                curveptw[3] += (basis[i] * self._mWeights[span - self._mDegree + i])
            # Divide by weight
            curvept = [float(curveptw[0] / curveptw[3]), float(curveptw[1] / curveptw[3]), float(curveptw[2] / curveptw[3])]
            self._mCurvePts.append(curvept)
